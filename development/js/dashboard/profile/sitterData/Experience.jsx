...
export default function Experience({ experience, id }) {
  ...
  const handleClick = (e) => {
    ...
    const getData = async () => {
      ...
      if (data) {
        inputRef.current.value = data.experience;
      }
    };
    getData();
  };
  ...
  const handleSave = (e) => {
    ...
    if (inputRef.current.value === "") {
      ...
      const saveChanges = async () => {
        const { data, error } = await supabase
          .from("sitter_form")
          .update({ experience: inputRef.current.value })
          .eq("uuid", id)
          .select("experience");
  ...
  return (
    <>
      {experience && (
        <>
          {!clicked ? (
            <p>{!newData ? experience : newData}</p>
          ) : (
            <>
